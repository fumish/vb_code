...
TRUE_K = length(hop);
x_data = zeros(max(car_number),DATA_SET); %%car_numberはデータセットごとに可変にできるように最終的にがんばる
t_data = zeros(max(car_number),DATA_SET);
...
%     d_ratio = test_d_ratio;
    ratio(:,d) = d_ratio';
    [d_x_data, d_t_data, d_label, ~] = func_tasep_simulation_main_ring(d_ratio, hop, car_number(d), cell_num(d), time_length(d));
